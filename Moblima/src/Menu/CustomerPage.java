...
public class CustomerPage extends MenuPage {
	...
	public void PurchaseTicketsUpdate()
	{
		...
		for (int i = 0 ; i < showList.size(); i++) {
			...
			String cinemaType = MovieCineplexModel.getInstance().GetCineplexes().get(s.getCineplexID()).getListCinemas().get(s.getScreenNumber()).getCinemaCategory();
        	...
        	try {
				mgio.assignSeatsByMovie(movieBooked, showindex-1, customerEmail, customerName, phoneNumber, bookingID, numseats, firstseat);
