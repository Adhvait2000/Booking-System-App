...
public class ListMoviesPage extends MenuPage {
	...
	public void Update() {
    	...
    	if(seatsOK)
    	{
    		...
    		MovieTicket ticket = new MovieTicket(chosenShow.get3D(), chosenCinema.getCinemaCategory(), movieGoerCategory, isPublicHoliday);
        	...
        	if(confirmation.toLowerCase().equals("yes")) {
            	...
            	try {
    				...
    				mgio.assignSeatsByMovie(selectedMovie, showIndex, customerEmail, customerName, phoneNumber, TID, numseats, firstseat);
			...
			for(int j = 0; j < cinemaList.size(); ++j)
			{
				...
				for(int k = 0; k < showList.size(); ++k)
				{
					...
					if(currentMovie.getMovieName().equals(selectedMovie.getMovieName()))
					{
						...
						displayOptions.add(optionCounter, new MovieOption(cineplexList.get(i), cinemaList.get(j), showList.get(k), currentMovie));
