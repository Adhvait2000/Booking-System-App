...
public class MovieGoerIO {
	    ...
	    private static int numOfBookings = 0; //to count number of bookings previously made, count no. of rows in each customer file (each row contains details of a customer's booking)
	    ...
	    public void previousBookingCount() throws IOException, Exception { //class to count previous bookings made by customers
			...
			while((str = reader.readLine()) != null) { 
				numOfBookings++; //update num of bookings based on number of lines in the customer file (each line is a booking made by a customer) 
			}
	   ...
	   public void assignSeatsByMovie(Movie movie, int index, String customer_name, String emailCustomer, int mobile_number, String booking_id, int seat_num, String first_seat) throws IOException, Exception {
		   ...
		   try{
	    		...
	    		if(check==1){ //if new customer, create new movie goer and assign id in addition to creating a new booking
